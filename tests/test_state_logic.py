...
class TestStateLogic(unittest.TestCase):
    ...
    def _run_updates(self, inputs):
        ...
        for ratio, hr in inputs:
            final_state = state_logic.update_stress_state(
                ratio, hr, self.baseline_metrics, final_state, self.history
            )
        return final_state
    ...
    def test_initial_state_calm(self):
        ...
        # Ratio > 1.2, HR < 77.5
        state = self._run_updates([(1.6, 70)])
        self.assertEqual(self.history[-1], "Calm")
    ...
    def test_initial_state_warning_ratio(self):
        ...
        # Ratio < 1.2, HR < 77.5
        state = self._run_updates([(1.1, 70)])
    ...
    def test_initial_state_warning_hr(self):
        ...
        # Ratio > 1.2, HR > 77.5
        state = self._run_updates([(1.6, 80)])
        self.assertEqual(self.history[-1], "Warning")
        self.assertEqual(state, "Initializing")
    ...
    def test_initial_state_stress(self):
        ...
        # Ratio < 1.2, HR > 77.5
        state = self._run_updates([(1.1, 80)])
        self.assertEqual(self.history[-1], "Stress")
        self.assertEqual(state, "Initializing")

    def test_state_nan_input(self):
        """Test handling of NaN input."""
        state = self._run_updates([(np.nan, 70)])
        self.assertEqual(self.history[-1], "Uncertain (NaN)")
        state = self._run_updates([(1.5, np.nan)])
        self.assertEqual(self.history[-1], "Uncertain (NaN)")
    ...
    def test_state_missing_baseline(self):
        ...
        state = state_logic.update_stress_state(1.5, 70, {}, self.current_state, self.history)
    ...
    def test_persistence_change_to_calm(self):
        ...
        self.current_state = "Warning" # Start in a different state
        inputs = [(1.6, 70)] * self.persistence # Calm inputs
    ...
    def test_persistence_change_to_warning(self):
        ...
        inputs = [(1.1, 70)] * self.persistence # Warning (low ratio) inputs
        final_state = self._run_updates(inputs)
    ...
    def test_persistence_change_to_stress(self):
        ...
        self.current_state = "Calm"
        inputs = [(1.1, 80)] * self.persistence # Stress inputs
        final_state = self._run_updates(inputs)
        self.assertEqual(final_state, "Stress")

    def test_persistence_no_change_mixed_states(self):
        """Test state does not change with mixed tentative states in history."""
        self.current_state = "Calm"
        inputs = [(1.6, 70)] * (self.persistence // 2) + \
                 [(1.1, 80)] * (self.persistence - self.persistence // 2) # Mix of Calm and Stress
        final_state = self._run_updates(inputs)
        # State should remain Calm because the history wasn't consistently Stress
        self.assertEqual(final_state, "Calm")
        self.assertEqual(self.history[-1], "Stress") # Last tentative was Stress
